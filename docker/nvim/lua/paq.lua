...
local logpath = vim.fn.has 'nvim-0.8' == 1 and vim.fn.stdpath 'log'
...
local function report(op, name, res, n, total)
  ...
  vim.notify(
    (' Paq:%s %s %s'):format(count, messages[op][res], name),
...
local function new_counter()
  return coroutine.wrap(function(op, total)
    ...
    local summary = (
      ' Paq: %s complete. %d ok; %d errors;'
      .. (c.nop > 0 and ' %d no-ops' or '')
    )
...
local function clone(pkg, counter, sync)
  ...
  call_proc('git', args, nil, function(ok)
    if ok then
      ...
      return pkg.run and run_hook(pkg, counter, sync)
        or counter(pkg.name, 'ok', sync)
    else
...
local function walk_dir(path, fn)
  ...
  while handle do
    ...
    if not fn(path .. '/' .. name, name, t) then return end
  end
  return true
end
...
local function check_rm()
  ...
  for _, packdir in pairs({ 'start', 'opt' }) do
    walk_dir(cfg.path .. packdir, function(dir, name)
      if name == 'paq-nvim' then return true end
...
local function remove(p, counter)
  ...
  if ok then packages[p.name] = { name = p.name, status = 'removed' } end
end
...
local function register(args)
  ...
  if not args.url and args[1]:match '^https?://' then args.url = args[1] end
